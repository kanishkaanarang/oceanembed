...
5. **Validation (45 seconds):** Explain the serving v3 evaluation and historical v2 artifacts. The v3 result is 0.271°C / 0.103 PSU on a five-day GLORYS split ending at 900 m; it applies to that limited test split, not every displayed date or region, and is not independent satellite-to-Argo validation.
6. **About & Saved Places (30 seconds):** Explain the v3 feature inputs and dual heads, depth outputs, and local saved locations.
...
| Validation | Serving v3 test report and per-depth errors, matched baseline, historical v2 evaluation artifacts |
...
- The serving engine loads **v3 SE-ResNet**, using its saved preprocessing and dual temperature/salinity heads through **900 m**.
- Temperature and salinity both come from v3. Density and sound speed are derived. No 1000 m v3 prediction is fabricated; the 900 m temperature reference is interpolated from 700/1000 m.
